...
`~/Downloads`)와 읽기·쓰기로 공유됩니다. 파일·압축 다운로드 저장 창은 이 폴더에서 시작하며,
저장한 파일은 호스트 파일 관리자에서도 바로 확인할 수 있습니다. 그 외 컨테이너 내부 경로는
호스트의 같은 이름 경로와 공유되지 않습니다.
...
설치 후 파일 탐색기의 **연결 프로그램 → SSH Editor**로 텍스트/소스 파일을 열 수 있습니다.
파일의 **상위 폴더가 로컬 작업 폴더로 열리고, 해당 파일이 선택된 편집 탭**으로 표시됩니다.
더블클릭으로 열려면 운영체제에서 해당 파일 형식의 기본 앱을 SSH Editor로 선택하세요.
앱 왼쪽의 **로컬 → 폴더 열기 / 파일 열기**도 같은 작업 폴더 기능을 사용합니다.

등록 형식: TXT, Markdown, JSON, YAML, TOML, XML, HTML, CSS, JS/TS, Python,
Shell, C/C++, Rust, Go, Java, Ruby, PHP, SQL 및 주요 소스·설정 확장자.
로컬 편집은 UTF-8 텍스트(최대 16 MiB)를 지원하며 저장, 외부 변경 감지,
충돌 확인은 기존 편집 탭과 동일하게 동작합니다. 로컬 트리는 폴더 탐색과 파일 열기를 지원합니다.

확장자 등록 변경은 `make install`로 다시 설치해야 반영됩니다.
Linux 컨테이너 실행 환경에서는 파일 연결로 전달받은 파일의 상위 폴더를 함께 마운트합니다.
